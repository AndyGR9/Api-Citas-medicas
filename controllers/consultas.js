...
const detallesPOST = async (req = request, res = response) => {
    ...
    try {
        ...
        const pacientN = await Paciente.findOne({ cedula: cedula, estado: true })

        if (!pacientN) {
            return res.status(404).json({ error: 'El paciente al que desea abrir consulta no se encuentra registrado' });
        }

        const consulta = new Consulta({ nombre: pacientN.nombre, apellidos: pacientN.apellidos, cedula, edad: pacientN.edad, detallesPaciente });
...
const examenesPOST = async (req = request, res = response) => {
    ...
    try {
        ...
        const consulta = await Consulta.updateOne(
            { _id: id },
            { examenes: examenes }
        );
